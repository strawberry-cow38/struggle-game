...
// Active only when ToolMode == UrBoard. Cursor previews a 1-tile board
// footprint at the hovered tile. Red when blocked. LMB places via
// PlaceUrBoardCommand.
public partial class UrBoardDesignator : Node2D
{
    ...
    {
        ZIndex = 55;
        if (Tools is not null) this.BindInputToMode(Tools, m => m == ToolMode.UrBoard, ClearPreview);
    }

    public override void _UnhandledInput(InputEvent @event)
    {
        if (Host is null) return;
        if (Tools is null || Tools.Mode != ToolMode.UrBoard)
        {
    ...
    public override void _Draw()
    {
        ...
        bool ok = Host.CanPlaceUrBoard(_hoverTile);
