...
// Active only when ToolMode == Stockpile. LMB-drag draws a rect;
// on release posts CreateStockpileRectCommand. Phase 3 will route
// click-without-drag into a panel open instead of a fresh zone.
public partial class StockpileDesignator : Node2D
{
    private const int PixelsPerTile = SimConstants.PixelsPerTile;

    private static readonly Color FillColor = new(0.95f, 0.85f, 0.25f, 0.18f);
    private static readonly Color BorderColor = new(1.00f, 0.90f, 0.35f, 0.95f);
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        if (Tools is null || Tools.Mode != ToolMode.Stockpile)
        {
