...
// Active only when ToolMode == FloorDecon. LMB-drag draws a rect; on
// release posts DeconstructFloorsInRectCommand which queues one
// FloorDeconstruct job per wood floor in the rect. Floors hidden
// under walls are skipped by TryPostFloorDeconJob.
public partial class FloorDeconDesignator : Node2D
{
    private const int PixelsPerTile = SimConstants.PixelsPerTile;

    private static readonly Color FillColor = new(0.85f, 0.35f, 0.85f, 0.22f);
    private static readonly Color BorderColor = new(1.0f, 0.45f, 1.0f, 0.85f);
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        if (Tools is null || Tools.Mode != ToolMode.FloorDecon)
        {
        ...
        if (@event is InputEventMouseButton mb && mb.ButtonIndex == MouseButton.Left)
        {
            ...
            else if (_dragging)
            {
                ...
                Host.QueueCommand(new DeconstructFloorsInRectCommand(_startTile, _currentTile));
                _dragging = false;
                QueueRedraw();
                GetViewport().SetInputAsHandled();
            }
