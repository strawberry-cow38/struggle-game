...
// Active only when ToolMode == Harvest. LMB-drag draws a rect; on
// release posts HarvestInRectCommand which queues a Harvest job per
// crop in the rect at ≥75% growth (immature crops are silently skipped).
public partial class HarvestDesignator : Node2D
{
    private const int PixelsPerTile = SimConstants.PixelsPerTile;

    private static readonly Color FillColor = new(1.0f, 0.55f, 0.20f, 0.20f);
    private static readonly Color BorderColor = new(1.0f, 0.70f, 0.30f, 0.85f);
    ...
    {
        ZIndex = 52;
        if (Tools is not null) this.BindInputToMode(Tools, m => m == ToolMode.Harvest, ClearPreview);
    }

    public override void _UnhandledInput(InputEvent @event)
    {
        if (Host is null) return;
        if (Tools is null || Tools.Mode != ToolMode.Harvest)
        {
        ...
        if (@event is InputEventMouseButton mb && mb.ButtonIndex == MouseButton.Left)
        {
            ...
            else if (_dragging)
            {
                ...
                Host.QueueCommand(new HarvestInRectCommand(_startTile, _currentTile));
                _dragging = false;
                QueueRedraw();
                GetViewport().SetInputAsHandled();
            }
