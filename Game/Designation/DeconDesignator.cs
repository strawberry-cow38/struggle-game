...
public partial class DeconDesignator : Node2D
{
    ...
    private static readonly Color FillColor = new(1.0f, 0.55f, 0.15f, 0.22f);
    private static readonly Color BorderColor = new(1.0f, 0.70f, 0.25f, 0.85f);
    ...
    {
        ZIndex = 53;
        if (Tools is not null) this.BindInputToMode(Tools, m => m == ToolMode.Decon, ClearPreview);
    }

    public override void _UnhandledInput(InputEvent @event)
    {
        if (Host is null) return;
        if (Tools is null || Tools.Mode != ToolMode.Decon)
        {
        ...
        if (@event is InputEventMouseButton mb && mb.ButtonIndex == MouseButton.Left)
        {
            ...
            else if (_dragging)
            {
                ...
                Host.QueueCommand(new DeconstructWallsInRectCommand(_startTile, _currentTile));
                _dragging = false;
                QueueRedraw();
                GetViewport().SetInputAsHandled();
            }
