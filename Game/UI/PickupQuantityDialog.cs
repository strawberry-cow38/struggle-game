...
// "Pick up X" dialog: a slider + number box (synced) capped at how many
// the chosen colonist can carry. Confirm posts a PickUpItemCommand.
public partial class PickupQuantityDialog : CanvasLayer
{
    ...
    {
        Layer = 96;
        _popup = new PopupPanel { Name = "PickupPopup" };
        AddChild(_popup);
        ...
        _title = new Label { Text = "Pick up", HorizontalAlignment = HorizontalAlignment.Center };
        ...
        var ok = new Button { Text = "Pick up", CustomMinimumSize = new Vector2(80, 28) };
        ok.Pressed += OnConfirm;
        btns.AddChild(ok);
        vbox.AddChild(btns);
    }

    public void Open(int pawnId, int itemId, int maxUnits, string itemName)
    {
        if (maxUnits < 1) return;
        _pawnId = pawnId;
        _itemId = itemId;
        _title.Text = $"Pick up {itemName} (max {maxUnits})";
        _sync = true;
    ...
    private void OnConfirm()
    {
        ...
        if (Host is not null && count > 0)
            Host.QueueCommand(new PickUpItemCommand(_pawnId, _itemId, count));
        _popup.Hide();
    }
}
