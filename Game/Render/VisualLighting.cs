...
public partial class VisualLighting : Node2D
{
    ...
    public void Tick()
    {
        ...
        {
            var noRoof = Host.CopyNoRoofTilesForRender();
            RebuildSunMask(noRoof);
            _lastRoofVersion = snap.RoofVersion;
        }

        // Lamp set/colors only need refreshing when the snapshot itself
        // changes; we deliberately don't re-create nodes per frame.
    ...
    private void RebuildSunMask(byte[] noRoof)
    {
        if (_sun is null) return;
        // White where outdoor (no-roof == 1), black elsewhere. Light2D
        // texture brightness modulates how much light hits each pixel,
        // so roofed tiles get zero sun contribution.
        ...
        for (int y = 0; y < MapHeight; y++)
        {
            ...
            for (int x = 0; x < MapWidth; x++)
            {
                img.SetPixel(x, y, noRoof[row + x] != 0 ? lit : dark);
            }
        }
    ...
    private void UpdateLamps(SimSnapshot snap)
    {
        ...
        foreach (var lamp in snap.Lamps)
        {
            ...
            // Lamp throw radius ~7 tiles. Texture is 128px; scale to
            // (7 tiles * PixelsPerTile * 2) / 128 so the falloff fills
            // roughly that range.
            const float LampRangeTiles = 7f;
